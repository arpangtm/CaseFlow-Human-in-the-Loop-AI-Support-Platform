...
class HumanReviewIntegrationTest {
    ...
    @Test
    void recordsOneHumanDecisionWithoutSendingOrResolvingTheCase() throws Exception {
                ...
                """
                        {
                          ...
                          "reviewerId": "%s",
                          "decision": "EDITED",
                          "editedResponse": "Please request a new password reset link from Account Settings."
                        }
                        """.formatted(DEMO_ORGANIZATION_ID, DEMO_REVIEWER_ID)
        );

        assertThat(reviewResponse.statusCode()).isEqualTo(201);
        assertThat(reviewResponse.body()).contains(
                "\"reviewerName\":\"Demo Agent\"",
                "\"decision\":\"EDITED\"",
                "\"finalResponse\":\"Please request a new password reset link from Account Settings.\"",
                "\"reviewLatencyMs\":"
        );

        HttpRequest getReviewRequest = HttpRequest.newBuilder()
                .uri(apiUri("/api/v1/recommendations/" + recommendationId
                        + "/review?organizationId=" + DEMO_ORGANIZATION_ID))
                .GET()
                .build();
        HttpResponse<String> getReviewResponse = client.send(
                getReviewRequest,
                HttpResponse.BodyHandlers.ofString()
        );
        assertThat(getReviewResponse.statusCode()).isEqualTo(200);
        assertThat(getReviewResponse.body()).contains("\"decision\":\"EDITED\"");
        ...
        HttpResponse<String> crossTenantResponse = postJson(
                client,
                "/api/v1/recommendations/" + recommendationId + "/review",
                """
                        {
                        ...
                        """.formatted(OTHER_ORGANIZATION_ID, DEMO_REVIEWER_ID)
        );
        assertThat(crossTenantResponse.statusCode()).isEqualTo(404);

        assertThat(jdbcTemplate.queryForObject(
                "SELECT count(*) FROM human_review_decisions WHERE recommendation_id = ?::uuid",
                Long.class,
                recommendationId
        )).isEqualTo(1L);
        assertThat(jdbcTemplate.queryForObject(
                "SELECT status FROM ai_recommendations WHERE id = ?::uuid",
                String.class,
                recommendationId
        )).isEqualTo("REVIEWED");
        assertThat(jdbcTemplate.queryForObject(
                "SELECT status FROM support_cases WHERE id = ?::uuid",
                String.class,
                caseId
        )).isEqualTo("NEW");
    }
